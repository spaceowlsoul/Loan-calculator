...
if args.type == "annuity":
    if args.principal and args.payment and args.interest:
        ...
        monthly_payment = math.ceil(args.principal * (nominal_interest_rate
                                                      * pow((one + nominal_interest_rate), args.periods))
                                    / (pow((one + nominal_interest_rate), args.periods) - one))
        overpayment = (args.periods * monthly_payment) - args.principal
        print(f'Your monthly payment = {monthly_payment}!')
        print(f'Overpayment = {overpayment}!')
    elif args.payment and args.periods and args.interest:
        nominal_interest_rate = args.interest / (months_in_year * hundred_percent)
        loan_principal = math.floor(args.payment / ((nominal_interest_rate * pow((one + nominal_interest_rate), args.periods))
                                                    / (pow((one + nominal_interest_rate), args.periods) - one)))
